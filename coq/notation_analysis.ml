...
let set_notation_var_scope ?loc id ((tmp_scope, subscopes) as scopes) ntnbinders
  ...
  try
    ...
    | Notation_term.NtnInternTypeAny principal ->
      ...
      in
      let () =
        match !ntn_binding_ids with
        | None -> ntn_binding_ids := Some ntnbinders
        | Some ntnbinders' ->
          ntn_binding_ids := Some (Id.Set.inter ntnbinders ntnbinders')
      in
      ()
...
let glob_local_binder_of_extended =
  DAst.with_loc_val (fun ?loc -> function
    ...
    | GLocalPattern (_, _, _, _) ->
      Loc.raise ?loc
        (Gramlib.Grammar.Error "Pattern with quote not allowed here."))
...
let find_projection_data c =
  ...
  | GApp (r, l) -> (
    ...
    | GRef (GlobRef.ConstRef cst, us) ->
      Some (cst, us, l, Structure.projection_nparams cst - List.length l)
    | _ -> None)
  | GRef (GlobRef.ConstRef cst, us) ->
    Some (cst, us, [], Structure.projection_nparams cst)
  | _ -> None
...
let drop_notations_pattern (test_kind_top, test_kind_inner) genv env pat =
  ...
  and in_patargs ?loc scopes gr (* head of the pattern *) expanded
    ...
    in
    let subscopes =
      adjust_to_down tags (List.skipn_at_best n (find_arguments_scope gr)) []
    in
    let has_letin =
  ...
  let rec intern env =
    CAst.with_loc_val (fun ?loc -> function
      ...
      | CGenarg gen ->
        ...
        Id.Map.iter
          (fun x (used_as_binder, _, _, _) -> used_as_binder := false)
          ntnvars;
        ...
        let match_to_do, nal =
          let rec canonize_args case_rel_ctxt arg_pats forbidden_names match_acc
            ...
            | ( (LocalAssum (cano_name, ty) | LocalDef (cano_name, _, ty)) :: t
              ...
              | _ ->
                let fresh =
                  Namegen.next_name_away_with_default_using_types "iV"
                    cano_name.binder_name forbidden_names (EConstr.of_constr ty)
                in
                canonize_args t tt
