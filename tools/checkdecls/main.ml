...
let cd_main roots debug files coqlib findlib_config ocamlpath rload_path
    load_path require_libraries =
...
let cd_cmd : int Cmd.t =
  let doc = "CheckDecls for Rocq" in
  let man =
    [ `S "DESCRIPTION"
  ...
  let fcc_term =
    ...
    Term.(
      const cd_main $ roots $ debug $ files $ coqlib $ findlib_config
      $ ocamlpath $ rload_paths $ qload_paths $ ri_from)
  in
  let exits = Exit_codes.[ missing_id; stopped; uri_failed ] in
  Cmd.(v (Cmd.info "rocq-checkdecls" ~exits ~version ~doc ~man) fcc_term)
